# Endpoint Collector
The **Endpoint Collector** is a free Solution to extend the CoE Starter Kit and automatically store the endpoints that the cloud flows from the tenant send HTTP requests to. The supported connectors are:
- HTTP
- HTTP Webhook
- HTTP Trigger (When an HTTP Request is)
- HTTP With Entra Id (both preauthorized and non-preauthorized)
