...
The **Solution Deployer** is a solution built to allow Power Platform CoEs to centrally maintain reusable components and deploy them widely across the organization, without the need for Managed Environments and premium licenses for all environment users. The main functionalities of the solution are:

- Configure the Target Environments to deploy solutions to
- Request the deployment of a solution to a single environment
- Manage Solution Subscriptions, mapping a solution with multiple subscribed environments to trigger the solution deployment to all related environments in one click
...
- The following connectors need to be enabled on the Environment: Dataverse, Office 365 for Outlook, Power Apps for Admins, Power Automate for Admins, Power Platform for Admins. 
...
## The Solution
The Solution contains around 30 components in total, principally composed of:
...
- A Command Library to leverage Power Fx from a custom command
...
5. On the left on the queries pane, click the Parameter "CoEKitOrgUrl", and replace the value by the Instance Url of the environment where the CoE Kit is installed, then clik Apply
6. Once replaced, open the the "admin_environment" query and set up the connection by clicking "Configure connection"
